...
def plot_pleio_genes(pleio_genes, all_genes, std_exp_data, output_dir):
    ...
    gene_name_map = all_genes[['gene_id_pos', 'gene_name_pos', 'gene_name_eqtl']].drop_duplicates(subset=['gene_id_pos'])
    df_plot = df_plot.merge(gene_name_map, left_on='Gene', right_on='gene_id_pos', how='left')
    df_plot['gene_name_pos'] = df_plot['gene_name_pos'].combine_first(df_plot['gene_name_eqtl'])
    ...
    phen_cat_map = all_genes[['phen_name', 'category']].drop_duplicates(subset=['phen_name'])
    df_plot = df_plot.merge(phen_cat_map, left_on='Phenotype', right_on='phen_name', how='left')
    ...
    df_plot['Source'] = pd.Categorical(df_plot['Source'], categories=custom_source_order, ordered=True)
    genes = df_plot[['gene_name_pos', 'Source']].drop_duplicates().sort_values(['Source', 'gene_name_pos'])['gene_name_pos'].tolist()
    phenotypes = df_plot[['Phenotype', 'category']].drop_duplicates().sort_values(['category', 'Phenotype'])['Phenotype'].tolist()


    color_map = {"Blood biochemistry": "#3C5488", "Blood count":'#DC0000',
                 "Medical conditions":'#631879', "Eyesight": '#91D1C2', 
                 "Sun exposure":"#C59316", "Visual acuity":"#4DBBD5", 
                 "Spirometry":"#E64B35",}
    ...
    heatmap_data = std_exp_data.set_index("Description").drop(columns=["ENSG", "Name"]).T
    ...
    # Plotting
    set_style()
    fig, (ax, ax_b) = plt.subplots(1, 2, figsize=(15, 9), gridspec_kw={'wspace': 1, 'width_ratios': [1, 1]})
    ...
    # Plot empty squares 
    ax.scatter(bg_genes, bg_phens, facecolors='none', edgecolors='black', 
            s=100, marker='s', linewidths=0.5, zorder=1, alpha=0.2)
    

    for i in range(len(phenotypes)):
        if i % 2 == 0:
            ax.axhspan(i - 0.5, i + 0.5, facecolor='#F4F4F4', zorder=0, edgecolor='none')
    
    
    edge_alpha = 0.2
    for category, color in color_map.items():
        subset = df_plot[df_plot['category'] == category]
        ax.scatter(subset['gene_name_pos'], subset['Phenotype'],
                c=color, s=100, marker='s', 
                edgecolors=(0, 0, 0, edge_alpha), linewidths=0.5, zorder=3)


    # Format axes and aesthetics
    ax.set_xticks(range(len(genes)))
    ax.xaxis.tick_top() 
    xtick_labels = ax.set_xticklabels(genes, rotation=45, ha='left', va='bottom')

    track_height = 0.4 # Adjust this to make the rectangle thinner or thicker
    for i, gene in enumerate(genes):
        source = gene_to_source.get(gene)
        color = source_color_map.get(source, 'lightgray')
        
        # Draw a rectangle for each gene. 
        # y starts at -0.5 (the top edge of the matrix). Since the y-axis is inverted, we subtract to go UP.
        rect = mpatches.Rectangle(
            (i - 0.5, -0.5 - track_height), 
            width=1.0, 
            height=track_height, 
            facecolor=color, 
            edgecolor='white', # Thin white line between segments to match your matrix tiles
            linewidth=0.5,
            clip_on=False      # Allows the rectangles to draw outside the strict plot limits
        )
        ax.add_patch(rect)
    ...
    ax.set_ylim(len(phenotypes) - 0.5, -0.5) # Swapping these values forces top-to-bottom sorting
    ...
    ax.tick_params(axis='x', which='major', length=5, width=0.6, direction='out', color='black', pad=5)


    # Replaces the legend and save section at the end of your function:
    handles_cat = [mpatches.Patch(color=color, label=str(label).capitalize()) for label, color in color_map.items()]
    legend_cat = ax.legend(handles=handles_cat, title='Category', bbox_to_anchor=(1.02, 1), 
                           loc='upper left', frameon=False, fontsize=8,
                           title_fontproperties={'weight': 'bold', "size": 10})
    ax.add_artist(legend_cat) 

    handles_source = [mpatches.Patch(color=color, label=label) for label, color in source_color_map.items()]
    legend_source = ax.legend(handles=handles_source, title='Mapping', bbox_to_anchor=(1.02, 0.6), 
                              loc='upper left', frameon=False, fontsize=8, 
                              title_fontproperties={'weight': 'bold', "size": 10})
    ...
    ax_b.set_xticklabels(heatmap_data.columns, rotation=45, ha="left")
    ax_b.set_yticklabels(heatmap_data.index)

    ax_b.tick_params(axis="x", top=True, bottom=False, length=5, width=0.6)
    ...
    cbar = fig.colorbar(
        c, ax=ax_b, location="left", shrink=0.4, aspect=30, pad=0.08
    )
    cbar.outline.set_visible(False)
    
    # Rotation changed to 90 so it reads correctly from bottom-to-top on the left side
    cbar.set_label("Median Expression (Z-score)", rotation=90, labelpad=15)
    ...
    plt.savefig(f'{output_dir}/pleiotropy_matrix.pdf', dpi=600, bbox_inches='tight', 
...
if __name__ == "__main__":
    ...
    pleiotropic_pos_df = all_genes_df.groupby('gene_id_pos').filter(lambda group: group['category'].nunique() > 1)
    pleiotropic_eqtl_df = all_genes_df.groupby('gene_id_eqtl').filter(lambda group: group['category'].nunique() > 1)
    ...
    pos_phenotypes = pleiotropic_pos_df.groupby('gene_id_pos')['phen_name'].unique().apply(list).to_dict()
    eqtl_phenotypes = pleiotropic_eqtl_df.groupby('gene_id_eqtl')['phen_name'].unique().apply(list).to_dict()
